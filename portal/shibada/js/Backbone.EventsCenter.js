...
var AppView = Backbone.View.extend({
    ...
    EventsCenter.CommanQuery["Combine"] = "DRETITLE";
    ...
    chart = new Highcharts.Chart({
        ...
        yAxis: {
            ...
            tooltip: {
                formatter: function() {
                    return '<b>' + this.series.name + (this.series.name == "微博" ? "(包含转发)" : "") + '</b>' +
        ...
        $.post("../Handler/Search.ashx", EventsCenter.staticsTotalCount, function(data) {
            var timeDate = getTrimeData(data.title, data.datas, false);
        ...
        EventsCenter.staticsTotalCount["text"] = unescape(EventsCenter.DaoDuModel.get("keywords"));
        $.post("../Handler/Search.ashx", EventsCenter.staticsTotalCount, function(data) {
            var timeDate = getTrimeData(data.title, data.datas, true);
...
function getTrimeData(cata, data, isWeibo) {
    var d = new Date();
    var resData = [];
    ...
    for (var i = 0; i <= cata.length; i++) {
        while (true) {
            ...
            if (getAddTime(count, mode) == cata[i]) {
                resData[count] = isWeibo ? (data[i] * 100) : data[i];
                count++;
                break;
            } else {
                resData[count] = 0;
